...
resp = requests.get(dgov_state_url, headers=headers)
min_data = resp.json()
...
def iterate_data(data, parent_key, idp_parent_id, state):
    for key in list(data.keys()):
        ...
        try:
            ...
            if data[key]["parent"] != "0" and parent_key == "0":
                idp_parent_id = create_org(data[key]["key"], data[key]["key"], "state", "STATE_GOVERNMENT", "DEPARTMENT", idp_parent_id)
                
            if data[key]["parent"] != "0" and parent_key != "0":
                idp_parent_id = create_org(data[key]["key"], data[key]["key"], "state", "STATE_GOVERNMENT", "ORGANISATION", idp_parent_id)                         
                
            if "child" in data[key]:
                iterate_data(data[key]["child"], data[key]["parent"], idp_parent_id, state)
